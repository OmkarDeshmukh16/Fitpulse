...
import { Apple, Flame, Beef, Wheat, Droplets, Calendar, Clock, Calculator } from 'lucide-react'
import { useGetPortalDietPlanQuery } from '../../services/portal.api'
import NutritionCalculator from '../../components/portal/NutritionCalculator'
    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div
        style={{
          ...
          gap: '0.5rem',
          marginBottom: '1.5rem',
          borderBottom: '1px solid var(--color-bg-border, rgba(255,255,255,0.08))',
          paddingBottom: '0.5rem',
        }}
      >
        <button
          className={`btn ${activeSubTab === 'plan' ? 'btn-primary' : 'btn-ghost'}`}
          onClick={() => setActiveSubTab('plan')}
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem' }}
        >
          <Apple size={16} /> My Active Plan
        </button>
        <button
          className={`btn ${activeSubTab === 'calculator' ? 'btn-primary' : 'btn-ghost'}`}
          onClick={() => setActiveSubTab('calculator')}
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem' }}
        >
          <Calculator size={16} /> Nutrition Calculator
        </button>
      </div>
      ...
      {activeSubTab === 'calculator' ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '50vh', gap: '1rem' }}>
          ...
          >
            <Calculator size={16} /> Calculate Your Estimated Needs
          </button>
        </div>
      ) : (
        <>
