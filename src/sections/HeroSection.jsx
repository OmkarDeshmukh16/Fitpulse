...
export default function HeroSection({ onOpenDemo }) {
  return (
    <section
      ...
      style={{
        paddingTop: 60,
        paddingBottom: 'clamp(16px, 4vw, 60px)',
        paddingLeft: 'clamp(16px, 4vw, 80px)',
        paddingRight: 'clamp(16px, 4vw, 80px)',
      }}
    >
      <div className="max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center min-h-[calc(100vh-72px)] py-4 md:py-8">

          {/* LEFT: Content */}
          <div className="flex flex-col gap-5 relative z-10 max-w-2xl">
            ...
            {/* Headline */}
            <motion.h1
              className="text-[34px] sm:text-[44px] md:text-[52px] xl:text-[60px] font-bold leading-[1.04] tracking-tight text-white"
            ...
            {/* Subtitle */}
            <motion.p
              className="text-base md:text-[18px] leading-[1.65] text-white/60 max-w-[600px] font-normal"
              style={{ fontFamily: 'Satoshi, sans-serif' }}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            {/* Quick stats row */}
            <motion.div
              className="grid grid-cols-3 gap-4 pt-4 border-t border-white/10 mt-1"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              {[
                ...
                <div key={s.label}>
                  ...
                  <div className="text-white/40 text-xs mt-0.5" style={{ fontFamily: 'Satoshi, sans-serif' }}>{s.label}</div>
                </div>
              ))}
            </motion.div>
          </div>
